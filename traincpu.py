...
if retrain:
    for iter_ in range(max_iteration):
        ...
        if (iter_)%log_interval==0 or iter_ in log_iters:
    ...
    # save P
    np.save(f'{result_dir}/P_{model.name}.npy', P)
...
def plot_train():
    ...
    print(f"saved in {figure_dir}/CEloss{model.name}s_{s_len}")
    # plt.show()
